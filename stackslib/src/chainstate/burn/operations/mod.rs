...
/// BitcoinZ-specific burn operations
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub enum BitcoinZOperationType {
    /// BitcoinZ burn operation
    Burn(BitcoinZBurnOp),
    /// BitcoinZ leader block commit
    LeaderBlockCommit(bitcoinz_burn::BitcoinZLeaderBlockCommitOp),
    /// BitcoinZ stack STX operation
    StackStx(bitcoinz_burn::BitcoinZStackStxOp),
}

/// Combined operation type that can handle both Bitcoin and BitcoinZ operations
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub enum BurnchainOperationType {
    /// Bitcoin-based operations (original Stacks)
    Bitcoin(BlockstackOperationType),
    /// BitcoinZ-based operations (BTCZS)
    BitcoinZ(BitcoinZOperationType),
}
...
impl BitcoinZOperationType {
    /// Get the transaction ID for this operation
    pub fn txid(&self) -> &Txid {
        match self {
            BitcoinZOperationType::Burn(op) => &op.txid,
            BitcoinZOperationType::LeaderBlockCommit(op) => &op.txid,
            BitcoinZOperationType::StackStx(op) => &op.txid,
        }
    }
    ...
    pub fn block_height(&self) -> u64 {
        match self {
            BitcoinZOperationType::Burn(op) => op.block_height,
            BitcoinZOperationType::LeaderBlockCommit(op) => op.block_height,
            BitcoinZOperationType::StackStx(op) => op.block_height,
        }
    }
    ...
    pub fn burn_amount(&self) -> u64 {
        match self {
            BitcoinZOperationType::Burn(op) => op.burn_amount,
            BitcoinZOperationType::LeaderBlockCommit(op) => op.burn_fee,
            BitcoinZOperationType::StackStx(_) => 0, // Stacking doesn't burn
        }
    }
    ...
    pub fn check(&self) -> Result<(), Error> {
        match self {
            BitcoinZOperationType::Burn(op) => op.check(),
            BitcoinZOperationType::LeaderBlockCommit(op) => op.check(),
            BitcoinZOperationType::StackStx(op) => op.check(),
        }
    }
}

impl BurnchainOperationType {
    /// Get the transaction ID for this operation
    pub fn txid(&self) -> &Txid {
        match self {
            BurnchainOperationType::Bitcoin(op) => op.txid_ref(),
            BurnchainOperationType::BitcoinZ(op) => op.txid(),
        }
    }
    ...
    pub fn block_height(&self) -> u64 {
        match self {
            BurnchainOperationType::Bitcoin(op) => op.block_height(),
            BurnchainOperationType::BitcoinZ(op) => op.block_height(),
        }
    }
    ...
    pub fn burn_amount(&self) -> u64 {
        match self {
            BurnchainOperationType::Bitcoin(op) => {
                // Extract burn amount from Bitcoin operations
                match op {
                    BlockstackOperationType::LeaderBlockCommit(commit_op) => commit_op.burn_fee,
                    _ => 0,
                }
            }
            BurnchainOperationType::BitcoinZ(op) => op.burn_amount(),
        }
    }
    ...
    pub fn check(&self) -> Result<(), Error> {
        match self {
            BurnchainOperationType::Bitcoin(_op) => {
                // Bitcoin operations have their own validation
                // For now, assume they're valid
                Ok(())
            }
            BurnchainOperationType::BitcoinZ(op) => op.check(),
        }
    }
}
