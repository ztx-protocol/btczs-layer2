...
use self::bitcoinz::{
    BitcoinZBlock, BitcoinZNetworkType, BitcoinZTransaction,
    Error as btcz_error,
};
...
use crate::core::*;
use crate::core::{
    BITCOINZ_MAINNET_FIRST_BLOCK_HEIGHT, BITCOINZ_MAINNET_FIRST_BLOCK_HASH, BITCOINZ_MAINNET_FIRST_BLOCK_TIMESTAMP, BITCOINZ_MAINNET_INITIAL_REWARD_START_BLOCK,
    BITCOINZ_TESTNET_FIRST_BLOCK_HEIGHT, BITCOINZ_TESTNET_FIRST_BLOCK_HASH, BITCOINZ_TESTNET_FIRST_BLOCK_TIMESTAMP,
    BITCOINZ_REGTEST_FIRST_BLOCK_HEIGHT, BITCOINZ_REGTEST_FIRST_BLOCK_HASH, BITCOINZ_REGTEST_FIRST_BLOCK_TIMESTAMP,
};
...
impl BurnchainParameters {
    pub fn from_params(chain: &str, network: &str) -> Option<BurnchainParameters> {
        match (chain, network) {
            ...
            ("bitcoinz", "mainnet") => Some(BurnchainParameters::bitcoinz_mainnet()),
            ("bitcoinz", "testnet") => Some(BurnchainParameters::bitcoinz_testnet()),
            ("bitcoinz", "regtest") => Some(BurnchainParameters::bitcoinz_regtest()),
            _ => None,
        }
    }
    ...
    pub fn bitcoinz_mainnet() -> BurnchainParameters {
        BurnchainParameters {
            chain_name: "bitcoinz".to_string(),
            network_name: bitcoinz::indexer::BITCOINZ_MAINNET_NAME.to_string(),
            network_id: bitcoinz::indexer::BITCOINZ_MAINNET,
            stable_confirmations: 6, // BitcoinZ has faster blocks, so fewer confirmations needed
            consensus_hash_lifetime: 24,
            first_block_height: BITCOINZ_MAINNET_FIRST_BLOCK_HEIGHT,
            first_block_hash: BurnchainHeaderHash::from_hex(BITCOINZ_MAINNET_FIRST_BLOCK_HASH)
                .unwrap(),
            first_block_timestamp: BITCOINZ_MAINNET_FIRST_BLOCK_TIMESTAMP,
            initial_reward_start_block: BITCOINZ_MAINNET_INITIAL_REWARD_START_BLOCK,
        }
    }

    pub fn bitcoinz_testnet() -> BurnchainParameters {
        BurnchainParameters {
            chain_name: "bitcoinz".to_string(),
            network_name: bitcoinz::indexer::BITCOINZ_TESTNET_NAME.to_string(),
            network_id: bitcoinz::indexer::BITCOINZ_TESTNET,
            stable_confirmations: 6,
            consensus_hash_lifetime: 24,
            first_block_height: BITCOINZ_TESTNET_FIRST_BLOCK_HEIGHT,
            first_block_hash: BurnchainHeaderHash::from_hex(BITCOINZ_TESTNET_FIRST_BLOCK_HASH)
                .unwrap(),
            first_block_timestamp: BITCOINZ_TESTNET_FIRST_BLOCK_TIMESTAMP,
            initial_reward_start_block: BITCOINZ_TESTNET_FIRST_BLOCK_HEIGHT,
        }
    }

    pub fn bitcoinz_regtest() -> BurnchainParameters {
        BurnchainParameters {
            chain_name: "bitcoinz".to_string(),
            network_name: bitcoinz::indexer::BITCOINZ_REGTEST_NAME.to_string(),
            network_id: bitcoinz::indexer::BITCOINZ_REGTEST,
            stable_confirmations: 1,
            consensus_hash_lifetime: 24,
            first_block_height: BITCOINZ_REGTEST_FIRST_BLOCK_HEIGHT,
            first_block_hash: BurnchainHeaderHash::from_hex(BITCOINZ_REGTEST_FIRST_BLOCK_HASH)
                .unwrap(),
            first_block_timestamp: BITCOINZ_REGTEST_FIRST_BLOCK_TIMESTAMP,
            initial_reward_start_block: BITCOINZ_REGTEST_FIRST_BLOCK_HEIGHT,
        }
    }
    ...
    pub fn is_testnet(network_id: u32) -> bool {
        matches!(
            ...
            bitcoinz::indexer::BITCOINZ_TESTNET | bitcoinz::indexer::BITCOINZ_REGTEST
        )
    }
}
...
impl BurnchainTransaction {
    pub fn txid(&self) -> Txid {
        match *self {
            ...
            BurnchainTransaction::BitcoinZ(ref btcz) => btcz.txid.clone(),
        }
    }
    ...
    pub fn vtxindex(&self) -> u32 {
        match *self {
            ...
            BurnchainTransaction::BitcoinZ(ref btcz) => btcz.vtxindex,
        }
    }
    ...
    pub fn opcode(&self) -> u8 {
        match *self {
            ...
            BurnchainTransaction::BitcoinZ(ref btcz) => btcz.opcode,
        }
    }
    ...
    pub fn data(&self) -> Vec<u8> {
        match *self {
            ...
            BurnchainTransaction::BitcoinZ(ref btcz) => btcz.data.clone(),
        }
    }
    ...
    pub fn num_signers(&self) -> usize {
        match *self {
            ...
            BurnchainTransaction::BitcoinZ(ref btcz) => btcz.inputs.len(),
        }
    }
    ...
    pub fn get_input_tx_ref(&self, input: usize) -> Option<&(Txid, u32)> {
        match self {
            ...
            BurnchainTransaction::BitcoinZ(ref btcz) => {
                btcz.inputs.get(input).map(|txin| &txin.tx_ref)
            }
        }
    }
    ...
    pub fn get_recipients(&self) -> Vec<Option<BurnchainRecipient>> {
        match *self {
            BurnchainTransaction::Bitcoin(ref btc) => btc
                ...
                .collect(),
            BurnchainTransaction::BitcoinZ(ref btcz) => btcz
                .outputs
                .iter()
                .map(|output| {
                    // For now, create a simple conversion from BitcoinZ output to BurnchainRecipient
                    // TODO: Implement proper BitcoinZ output to BurnchainRecipient conversion
                    None // Placeholder - will implement proper conversion later
                })
                .collect(),
        }
    }
    ...
    pub fn num_recipients(&self) -> usize {
        match *self {
            ...
            BurnchainTransaction::BitcoinZ(ref btcz) => btcz.outputs.len(),
        }
    }
    ...
    pub fn get_burn_amount(&self) -> u64 {
        match *self {
            ...
            BurnchainTransaction::BitcoinZ(ref btcz) => btcz.data_amt,
        }
    }
}
