...
impl BurnchainBlock {
    pub fn block_height(&self) -> u64 {
        match *self {
            ...
            BurnchainBlock::BitcoinZ(ref data) => data.block_height,
        }
    }
    ...
    pub fn block_hash(&self) -> BurnchainHeaderHash {
        match *self {
            ...
            BurnchainBlock::BitcoinZ(ref data) => data.block_hash.clone(),
        }
    }
    ...
    pub fn parent_block_hash(&self) -> BurnchainHeaderHash {
        match *self {
            ...
            BurnchainBlock::BitcoinZ(ref data) => data.parent_block_hash.clone(),
        }
    }
    ...
    pub fn txs(&self) -> Vec<BurnchainTransaction> {
        match *self {
            BurnchainBlock::Bitcoin(ref data) => data
                ...
                .collect(),
            BurnchainBlock::BitcoinZ(ref data) => data
                .txs
                .iter()
                .map(|tx| BurnchainTransaction::BitcoinZ((*tx).clone()))
                .collect(),
        }
    }
    ...
    pub fn timestamp(&self) -> u64 {
        match *self {
            ...
            BurnchainBlock::BitcoinZ(ref data) => data.timestamp,
        }
    }
    ...
    pub fn header(&self) -> BurnchainBlockHeader {
        match *self {
            ...
            BurnchainBlock::BitcoinZ(ref data) => BurnchainBlockHeader {
                block_height: data.block_height,
                block_hash: data.block_hash.clone(),
                parent_block_hash: data.parent_block_hash.clone(),
                num_txs: data.txs.len() as u64,
...
impl Burnchain {
    pub fn new(
        ...
        let (params, pox_constants, peer_version) = match (chain_name, network_name) {
            ...
            ("bitcoinz", "mainnet") => (
                BurnchainParameters::bitcoinz_mainnet(),
                PoxConstants::mainnet_default(),
                PEER_VERSION_MAINNET,
            ),
            ("bitcoinz", "testnet") => (
                BurnchainParameters::bitcoinz_testnet(),
                PoxConstants::testnet_default(),
                PEER_VERSION_TESTNET,
            ),
            ("bitcoinz", "regtest") => (
                BurnchainParameters::bitcoinz_regtest(),
                PoxConstants::regtest_default(),
                PEER_VERSION_TESTNET,
            ),
            (_, _) => {
